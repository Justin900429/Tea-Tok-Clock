...
plt.plot(data.iloc[:, 0], label="temperature")
...
model = LinearRegression().fit(
    data.iloc[:, 1].array.reshape(-1, 1), data["log_scale"].array.reshape(-1, 1)
)
y_pred = model.predict(np.arange(len(data)).reshape(-1, 1))
...
# Compute k value
data["k"] = -np.log((data.iloc[:, 0] - 23.0) / 96.812) / (data.iloc[:, 1] + 1)
